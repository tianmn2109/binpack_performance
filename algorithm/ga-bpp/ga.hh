...
int getMinBins() {
	int ret = population[0].num;
	for (int i = 0; i < POP_SIZE; i ++)
		if (population[i].num < ret)
			ret = population[i].num;
	return ret;
}
double fitness(struct bpp b) {
	double fit = 0.0;
	for (int i = 0; i < POP_SIZE; i ++) {
		double tempFit = 0.0;
		for (int j = 0; j < population[i].num; j ++) {
	//		cout << "popilation[i].cap[j] " << population[i].cap[j] << " b.c = " << b.c << endl;
			tempFit += (double)(b.c - population[i].cap[j]) / (double)b.c * (double)(b.c - population[i].cap[j]) / (double)b.c;
		}
		fit += tempFit / population[i].num;
	}
	return fit / POP_SIZE;
}
